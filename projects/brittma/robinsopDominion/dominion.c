...
int adventurerFunction(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int temphand[MAX_HAND], int z){
  while(drawntreasure<2){
    /// was <, now <=, and discardCount was deckCount
    if (state->discardCount[currentPlayer] <=1){//if the deck is empty we need to shuffle discard and add to deck
      shuffle(currentPlayer, state);
    }
    drawCard(currentPlayer, state);
    cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
    /// following were OR instead of AND, changed back so no segfault
...
int smithyFunction(int i, int currentPlayer, int handPos, struct gameState *state){
...
int stewardFunction(int currentPlayer, struct gameState *state, int choice1, int choice2, int choice3, int handPos){
  if (choice1 == 1)
  {
    //+2 cards
    drawCard(currentPlayer, state);
  ...
  else
  {
    ...
    /// following was choice3, now choice2
    discardCard(choice2, currentPlayer, state, 1);
  }
...
//      }
    ...
    case smithy: // todo
      smithyFunction(i, currentPlayer, handPos, state);
//      //+3 Cards
//      for (i = 0; i < 3; i++)
//	{
//	  drawCard(currentPlayer, state);
...
//      discardCard(handPos, currentPlayer, state, 0);
    ...
    case steward: // todo
      stewardFunction(currentPlayer, state, choice1, choice2, choice3, handPos);
//      if (choice1 == 1)
//	{
//	  //+2 cards
//	  drawCard(currentPlayer, state);
